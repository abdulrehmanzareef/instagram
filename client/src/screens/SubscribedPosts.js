...
import '../App.css';
            ...
            <div className="home">
            {
                data.map(item=>{
                    ...
                    return(
                       ...
                       <Link  className="link-setting"  to={item.postedBy._id !== state._id?"/profile/"+item.postedBy._id :"/profile"  }>{item.postedBy.name}
                       </Link>
                      
                       <span style={{float:"right",color:"blue"}} >{moment(item.createdAt).fromNow()}</span>
                       ...
                       ?<i className="material-icons"
                        ...
                        <br></br>
                         ...
                         <span>{item.comments.length} comments</span> 
                           <h6>{item.title}</h6>
                           {/* <p>{item.body}</p> */}
                           <h6><strong><Link onClick={scrollToTop } to={item.postedBy._id !== state._id?"/profile/"+item.postedBy._id :"/profile"  }>{item.postedBy.name }</Link> </strong>
          &nbsp;{item.body}</h6>
    ...
    <Link onClick={scrollToTop} to={record.postedBy._id !== state._id?"/profile/"+record.postedBy._id :"/profile"  }>{record.postedBy.name}</Link>  
                                            </span>{" "}
                                            {record.text}
